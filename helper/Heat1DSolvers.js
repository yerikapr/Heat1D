class Heat1DSolvers
{
    ...
    NeumannSolver(props)
    {
        ...
        temp[0] = lastTemp[0] + alpha*((this.#diffAr[1] - this.#diffAr[0])*(lastTemp[1] - lastTemp[0]) 
            + 2*this.#diffAr[0]*(lastTemp[1] - lastTemp[0] - this.boundary[0]*this.deltaX) );

        temp[this.#totalData-1] = lastTemp[this.#totalData-1] + alpha*((this.#diffAr[this.#totalData-1] - this.#diffAr[this.#totalData-2])*(2*this.deltaX*this.boundary[1]) 
